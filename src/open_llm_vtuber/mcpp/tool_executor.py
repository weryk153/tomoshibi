...
class ToolExecutor:
    def __init__(
        ...
        # Tools the caller has switched off for the rest of this turn, name ->
        # the reason the LLM gets back instead of a result. The point is that
        # the work never happens: a suppressed tool is answered here, before
        # MCPClient hands it to the server subprocess, so whatever side effect
        # the tool would have had (an HTTP submit, a file write) never runs.
        # Suppression is per-turn, not per-caller: clear_suppressions() empties
        # the whole table, so a second caller's entry goes with it. Fine while
        # one caller uses this; revisit before adding another.
        self._suppressed_tools: Dict[str, str] = {}

    def suppress_tool(self, tool_name: str, reason: str) -> None:
        """Answer `tool_name` with `reason` instead of running it.

        `reason` goes straight into the LLM's tool result, so write it as
        something the model can act on ("already done", not a stack trace).
        """
        self._suppressed_tools[tool_name] = reason

    def clear_suppressions(self) -> None:
        """Let every tool run again. Call this at the end of a turn."""
    ...
    async def run_single_tool(
        ...
        # Checked before get_tool and before call_tool: this is the only point
        # where a tool call can still be stopped without the tool having done
        # its work, because everything past here is a round trip to the server
        # process that owns the side effect.
        suppressed_reason = self._suppressed_tools.get(tool_name)
        if suppressed_reason is not None:
            logger.info(
                f"Tool '{tool_name}' is suppressed for this turn; "
                f"not executing it ({suppressed_reason})"
            )
            return (
                False,
                suppressed_reason,
                {},
                [{"type": "text", "text": suppressed_reason}],
            )
