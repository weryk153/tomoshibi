...
class BatchInput(BaseInput):
    ...
    Attributes:
        ...
        metadata: Optional dictionary of metadata flags for special inputs
            ...
            - 'previous_proactive_response': Latest proactive remark, supplied once
              to preserve continuity when the user replies to it
            - 'proactive_forbid_question': Suppress a new proactive question when
              the previous proactive turn already asked one
            - 'proactive_image_sources': Camera/screen sources used to reject
              observations that the supplied images cannot support
            - 'proactive_recent_outputs': Recent proactive lines used for
              sentence-level exact and near-duplicate filtering
            - 'proactive_context_uid': Durable conversation-history identity
              used to keep anti-repeat context across socket reconnects
            - 'proactive_visual_facts': Persona-free facts extracted from the
              supplied images before the character response
    """
