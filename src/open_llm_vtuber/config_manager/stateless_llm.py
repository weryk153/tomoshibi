...
class OpenAICompatibleConfig(StatelessLLMBaseConfig):
    ...
    _OPENAI_COMPATIBLE_DESCRIPTIONS: ClassVar[dict[str, Description]] = {
        ...
        "extra_body": Description(
            en="Extra provider-specific fields merged into the request body "
            "(e.g. reasoning_effort: none to disable a thinking model's reasoning).",
            zh="附加到請求主體的 provider 專屬欄位（例如 reasoning_effort: none 可關閉思考模型的推理）。",
        ),
    }
