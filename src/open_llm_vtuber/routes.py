...
def init_client_ws_route(
    ...
    Returns:
        tuple: (APIRouter configured with the WebSocket endpoint, the
            WebSocketHandler instance backing it). The handler is returned so
            other routers (e.g. live2d_config_route's PUT endpoint) can reach
            its live connection registry (``client_contexts``) to refresh
            per-session state after a config write — it is otherwise created
            and held only inside this function's closure.
    """
...
def init_webtool_routes(default_context_cache: ServiceContext) -> APIRouter:
    ...
    async def get_live2d_folder_info():
        ...
        for entry in os.scandir(live2d_dir):
            if entry.is_dir() and is_discoverable_live2d_dir(entry.name):
                ...
                for pattern in (
                    ...
                    matches = sorted(glob.glob(os.path.join(entry.path, pattern)))
                    if matches:
                        model3_file = matches[0].replace("\\", "/")
                        break

                if model3_file and os.path.isfile(model3_file):
