...
// startTapMotion 専用の純選択ロジック。Cubism 公式の LAppModel には無い、
// このプロジェクト独自の拡張（詳しくは startTapMotion 本体のコメント参照）。
import { selectTapMotion, type TapMotionsConfig } from "@/utils/tap-motion-selection";
...
export class LAppModel extends CubismUserModel {
  /**
   * ポインタ追従（視線・頭・体の向き）を有効にするか。設定画面のトグルが
   * 書き換える。アプリ全体の好みなのでインスタンスではなく static。
   */
      ...
      fetch(`${this._modelHomeDir}${modelFileName}`)
        ...
        .then((arrayBuffer) => {
          ...
          this.loadModel(
            ...
            this._modelScale ?? LAppDefine.CurrentKScale
          );
  ...
  public update(): void {
    ...
    // ドラッグ追従（視線・頭・体）のオン／オフ。設定画面のトグルから
    // LAppModel.lookAtPointer を書き換える。既定は true（SDK 標準の挙動）。
    //
    // 「マウスを押している間だけ顔がこちらを向く」のは SDK 標準だが、ドラッグで
    // キャラを移動させたいだけの人にとっては、動かすたびに顔が振り回されて
    // 落ち着かない。設定で切れるようにした。
    //
    // static にしているのは、この値がモデル個別ではなくアプリ全体の好みだから。
    // モデルを切り替えても設定は維持される。
    ...
    if (this._lipsync) {
      ...
      if (compactRig) {
        ...
        const blend = 1.0 - Math.exp(-rate * deltaTimeSeconds);
        this._smoothedLipSyncValue +=
          (targetValue - this._smoothedLipSyncValue) * blend;
        value = this._smoothedLipSyncValue;
      } else {
        value = targetValue;
      }
  ...
  /**
   * Smoothly leave a speaking or reaction motion and resume Idle.
   * Starting a force-priority motion makes Cubism fade the current motion out
   * using its configured FadeOutTime instead of releasing it abruptly.
   */
  public returnToIdleMotion(): CubismMotionQueueEntryHandle {
    return this.startRandomMotion(
      LAppDefine.MotionGroupIdle,
   ...
   * 現在の表情を解除して素の顔に戻す。
   *
   * Cubism の表情はパラメータへの加算・乗算なので「表情なし」という表情は
   * 存在しない。素に戻す唯一の方法は再生中の表情モーションを止めること。
   * stopAllMotions() 自体は CubismMotionQueueManager の public API だが、
   * LAppModel からは公開されていなかったので薄く包んでいる。
   *
   * 「中立の表情をひとつ選んで上から被せる」方式は採らない。どれが中立かを
   * 決める手段がないからで、以前は表情リストの 0 番目を使っていた——0 番目は
   * モデル作者が最初に並べただけの表情で中立とは限らず、フリーレンの 0 番は
   * 「泣き」だったため、発話が終わるたびに泣き顔に戻っていた。
   */
  ...
  public hitTest(hitArenaName: string, x: number, y: number): boolean {
    // 透明時は当たり判定無し。
    if (this._opacity < 1 || this._model == null || this._modelSetting == null) {
      return false;
    }
  ...
  public isHitOnModel(x: number, y: number): boolean {
    ...
    if (this._opacity < 1 || this._model == null || this._modelMatrix == null) {
      return false;
    }
   ...
   *
   * tapMotionsConfig の形は `{hitAreaId: [{group, index, weight}]}` に統一
   * 済み。以前はここが `{hitAreaId: {groupName: weight}}` で、選ぶ単位が
   * 「グループ」だったため常に startRandomMotion しか呼べなかった
   * （index を記録していなかったので特定の1本を狙い撃ちできなかった）。
   *
   * 意図的に旧形式との互換分岐は無い： GET /api/live2d/model-config の
   * バックエンド側で legacy データはこの新形式に正規化してから返るため、
   * フロントエンドが旧形式を目にすることはない。
   *
   * 実際の選択ロジック（重み付きランダムでどの候補を選ぶか）は純関数として
   * `@/utils/tap-motion-selection` に切り出してある——このファイルは
   * vendored な Cubism SDK サンプルコードで、`@framework/*` 経由の依存が
   * ないと単体テストできない。ロジックをここに残したままテストする方法が
   * ないため、テスト可能な形に外出しした。
   *
   ...
   */
  public startTapMotion(hitAreaName: string | null, tapMotionsConfig: TapMotionsConfig | null | undefined): void {
    if (LAppDefine.DebugLogEnable) {
    ...
    const decision = selectTapMotion(tapMotionsConfig, hitAreaName);

    if (LAppDefine.DebugLogEnable) {
      console.log('[APP] startTapMotion: Selected candidate:', decision);
    }

    if (decision === null) {
      if (LAppDefine.DebugLogEnable) {
        console.log('[APP] startTapMotion: Could not select a motion.');
      }
      return;
    }

    // Use PriorityForce (3) to ensure the motion plays. index が number なら
    // その1本を指定再生、null ならグループ内ランダム再生（改版前の
    // {groupName: weight} 形式の挙動をそのまま保持）。
    if (typeof decision.index === 'number') {
      this.startMotion(decision.group, decision.index, 3);
    } else {
      this.startRandomMotion(decision.group, 3);
    }
  }
