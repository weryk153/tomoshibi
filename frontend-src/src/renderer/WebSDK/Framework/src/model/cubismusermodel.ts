...
export class CubismUserModel {
  ...
  public isHit(
    ...
    // Model loading is asynchronous. During a character switch the LAppModel
    // wrapper already exists, but Cubism Core may not have created its internal
    // model yet (or may have rejected an incompatible/corrupt moc3). Pointer
    // events must be harmless in that state instead of dereferencing null.
    if (this._model == null || drawableId == null) {
      return false;
    }
