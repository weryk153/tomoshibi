...
export function VADProvider({ children }: { children: React.ReactNode }) {
  ...
  const handleVADMisfire = useCallback(() => {
    ...
    setSubtitleTextRef.current(t('error.vadMisfire'), 4000);
  }, [t]);
  ...
  const startMic = useCallback(async () => {
    try {
      ...
      // 語音輸入走 MicVAD → getUserMedia，所以「沒給麥克風權限」會以
      // DOMException 的形式落在這裡，而那是這條路最常見的失敗。認得出來就講
      // 使用者能做的事；認不出來（onnxruntime 載不起來、wasm 問題）就保留原本
      // 的例外文字——那時它是唯一的線索，換成一句通用的話反而幫不上忙。
      const micKey = microphoneErrorKey(error);
      toaster.create({
        title: micKey ? t(micKey) : `${t('error.failedStartVAD')}: ${error}`,
        type: 'error',
        duration: 6000,
      });
    }
