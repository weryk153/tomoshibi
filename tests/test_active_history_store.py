...
def test_active_history_round_trip(tmp_path, monkeypatch):
    ...
    active_history_store.set_active_history_uid("aoi", "2026-08-17_18-49-17_abc123")

    assert (
        active_history_store.get_active_history_uid("aoi")
    ...
    assert state == {
        "version": 1,
        "histories": {"aoi": "2026-08-17_18-49-17_abc123"},
    }
...
def test_active_history_is_tracked_per_character(tmp_path, monkeypatch):
    ...
    active_history_store.set_active_history_uid("aoi", "history-a")
    active_history_store.set_active_history_uid("task3", "history-b")

    assert active_history_store.get_active_history_uid("aoi") == "history-a"
...
def test_clearing_one_character_keeps_the_others(tmp_path, monkeypatch):
    ...
    active_history_store.clear_active_history_uid("aoi")

    assert active_history_store.get_active_history_uid("aoi") is None
    assert active_history_store.get_active_history_uid("task3") == "history-b"
...
def test_active_history_rejects_unsafe_uids(tmp_path, monkeypatch, history_uid):
    ...
    with pytest.raises(ValueError):
        active_history_store.set_active_history_uid("aoi", history_uid)
...
def test_a_poisoned_entry_does_not_leak_out(tmp_path, monkeypatch):
    ...
    (state_dir / ".active-history.json").write_text(
        json.dumps({"version": 1, "histories": {"aoi": "../../etc/passwd"}}),
        encoding="utf-8",
    )
