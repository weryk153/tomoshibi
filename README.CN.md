...
> 免费、开源、新手也能上手，让你**和喜欢的动漫角色聊天、互动**的 app。放进 Live2D 或 VRM（3D）模型，角色会根据对话自己做表情和动作，说话的语言也能设置，还会记得你、主动找你聊。自带 LLM，其余开箱即用。
...
>
> **下载 app：** 到 [Releases](https://github.com/weryk153/tomoshibi/releases/latest)（macOS／Windows）。也可以从源码运行。

## 最短路径

**下载 app**（最简单）

1. 到 [Releases](https://github.com/weryk153/tomoshibi/releases/latest) 下载：Apple Silicon Mac 选 `arm64.dmg`、Intel Mac 选 `x64.dmg`、Windows 选 `setup.exe`
2. 打开 app。第一次会自动下载需要的东西，只需一次，约几分钟。被系统拦住的话，看下方的安全提示说明。
3. 向导里点 **一键安装** → 挑 2D 或 3D 角色 → 开始聊天。

**不装 app** —— 大约 10 分钟，其中 8 分钟在等下载。

1. 在这个页面点绿色 **`<> Code`** → **Download ZIP** → 解压
2. 双击 `start-companion.command`（macOS）或 `start-companion.bat`（Windows）。
   **那个窗口别关，它就是服务器。**
3. 浏览器会自己打开 → 向导里点 **一键安装**（自动装好 Ollama 和免费的本地模型）→ 挑 2D 或 3D 角色 → 开始聊天。
...
**Tomoshibi** 让你喜欢的角色 —— 手绘的 Live2D 或全 3D 的 VRM —— 真的能跟你聊天、互动：它记得你、会自己开启话题、你说话时会听，说话时还会自己做表情和动作。

它是把优秀的开源项目 [Open-LLM-VTuber](https://github.com/Open-LLM-VTuber/Open-LLM-VTuber) **重新打包成对新手友好**的版本。我们站在它的肩膀上：上游提供稳定可靠的 Live2D + 语音识别/合成 + LLM 底层；这个 fork 在旁边补上 VRM（3D），并把它包装成「**下载 → 双击 → 开聊**」的体验，面向非技术用户，并加上了记忆系统、主动对话、自然插话语音、角色管理、应用内设置向导，以及五种语言的界面。
...
- **AI 驱动表情与动作** —— 每句话由 LLM 挑选表情和动作，并决定强弱（浅笑或大笑），Live2D 与 VRM 都支持。
- **可设置表达语言** —— 每个角色可设置回复语言（日语、英语……）。语音使用的语言不同时，会先翻译再朗读。
...
- **五种语言界面** —— English、繁體中文、简体中文、日本語、한국어。

---

## 截图

![Tomoshibi 运行画面](assets/tomoshibi-hero.png)
...
> - **（推荐 —— 免费、私密、跑在你自己的电脑上）通过 Ollama 用本地模型。** 设置向导点一下就会装好免费的 **[Ollama](https://ollama.com)** 应用和一个小模型（`qwen2.5:3b`，约 1.9 GB），不用敲命令。Tomoshibi 的默认本来就指向它，所以直接就能用 —— **不用账号、不用 API key、零费用、可离线，而且你的对话永远不会离开你的电脑。** 一般 8–16 GB 的笔记本就跑得动。（想要更聪明的回复、内存也够？拉一个更大的模型，例如 `qwen2.5:7b`，再到设置里选它。）
...
> **macOS Gatekeeper（仅第一次启动）：** 可能会提示「无法验证」这个应用。这对未签名的开源应用来说是正常的。先点 **完成**，再到 **系统设置 → 隐私与安全性** 点 **仍要打开**。app 和 `start-companion.command` 都一样。旧版 macOS 也可以右键 → **打开**。（我们不提供签名 / 公证版本 —— 这是免费档位。）
...
- **macOS：** 双击 **`start-companion.command`** —— 第一次系统可能会拦：点 **完成**，再到 **系统设置 → 隐私与安全性 → 仍要打开**；之后正常双击就行。
...
你需要**二者择一**：一个云端 LLM 的 API key，**或**一个正在运行的本地 LLM。聊天用便宜的模型就足够了 —— 不需要旗舰级。

#### 方案 A —— 本地 Ollama（推荐：免费、私密、不用账号）
在设置向导点 **一键安装**，会自动装好 [Ollama](https://ollama.com/download)、下载 `qwen2.5:3b`（约 1.9 GB）并切换过去 —— 不用敲命令、不用重启、不用 API key、不用账号、没有云端费用、完全离线，而且你的对话都留在你的电脑上。一般 8–16 GB 的笔记本就跑得很顺。想要更聪明的回复，可以拉一个更大的模型（例如 `qwen2.5:7b`），再到 LLM 设置标签页里选它。
...
**建议：** 选一个正常的（非思考型）对话模型。一个小而快的模型反而能让对话更自然、延迟更低。
...
### 主动话题与新闻
角色会在空闲一段时间后开启话题。可选用内置的新闻小工具（`scripts/news_topics.py`）用当前头条刷新这些话题 —— 纯标准库，无需 API key —— 并给它设定时（cron / launchd / 任务计划程序），例如每隔几小时一次。
...
**macOS：「无法打开」或「Apple 无法验证」。**
点 **完成**，再到 **系统设置 → 隐私与安全性 → 仍要打开**。只有第一次需要。（旧版 macOS 也可以右键 → **打开**。）
...
- [从手机 / 平板使用（Tailscale）](docs/remote-access-tailscale.md) —— 即使不在家里的网络，也能从另一台设备跟你的角色聊天。
...
- **Tomoshibi 自己新增的部分**（长期记忆、主动开口、群组对话、舞台演出、以及上述全部的应用内设置界面、五语言 UI）—— MIT。
